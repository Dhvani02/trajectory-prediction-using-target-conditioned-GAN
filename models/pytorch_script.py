...
class TrajPredictor(pl.LightningModule):
	...
	def generator_step(self, batch):
		...
		if self.hparams.absolute:
			l2 = self.loss_fns["L2"](
				batch["gt_xy"][:(batch['occupancy'][0][0]+1)], #because all trajecotires are not of the same length
				generator_out["out_xy"][:(batch['occupancy'][0][0]+1)],
				mode='average',
				type="mse")
		else:
			l2 = self.loss_fns["L2"](
				batch["gt_dxdy"][:(batch['occupancy'][0][0]+1)],
				generator_out["out_dxdy"][:(batch['occupancy'][0][0]+1)],
				mode='raw',
				type="mse")

		ade_error = cal_ade(
			batch["gt_xy"][:(batch['occupancy'][0][0]+1)], generator_out["out_xy"][:(batch['occupancy'][0][0]+1)], mode='raw'
		)
		...
		if self.generator.rm_vis_type == "attention":
			...
			image_patches = None
# 		print('batch["in_xy"][:, :batch_size]', batch["in_xy"][:, :batch_size].shape)
# 		print('batch["in_dxdy"][:, :batch_size]',batch["in_dxdy"][:, :batch_size])
# 		print('traj_fake',traj_fake.shape)
